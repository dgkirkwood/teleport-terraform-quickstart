...
variable "hostname" {
  description = "The hostname of your domain controller, for example 'dc01'"
}
...
locals {
  ...
  install_ca_command   = "Install-WindowsFeature Adcs-Cert-Authority -IncludeManagementTools"
  configure_ca_command = "Install-AdcsCertificationAuthority -CAType EnterpriseRootCa -CACommonName ${local.virtual_machine_fqdn} -ValidityPeriod Years -ValidityPeriodUnits 3  -Force"
  shutdown_command     = "shutdown -r -t 10"
  exit_code_hack       = "exit 0"
  ad_powershell_command   = "${local.import_command}; ${local.password_command}; ${local.install_ad_command}; ${local.configure_ad_command}; ${local.shutdown_command}; ${local.exit_code_hack}"
  ca_powershell_command = "${local.install_ca_command}; ${local.configure_ca_command};${local.exit_code_hack}"
}
